...
function redactarInforme(lev){
  ...
  if(lev.notas && lev.notas.trim()){
    P.push('Observaciones adicionales: ' + limpiarTexto(lev.notas.trim()));
  }
  return P;
}
...
async function generarPDF(lev, progreso){
  ...
  /* ---------- Hoja 1: detalle de la información (SIN texto redactado) ----------
     El texto redactado va únicamente en el reporte Word. Aquí solo la
     tabla de datos capturados y luego las fotos. */
  encabezado(doc, lev, 'Evidencias de levantamiento');
  piePagina(doc, lev, lev.folio + '  ·  Detalle');
  let y = TOPE_CONTENIDO;

  doc.setTextColor.apply(doc, TINTA); doc.setFont('helvetica','bold'); doc.setFontSize(16);
  doc.text('Evidencias de levantamiento', 15, y); y += 6.5;
...
function nombrePDF(lev){ return 'Evidencias_'+lev.folio+'.pdf'; }
