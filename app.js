...
/* ===================== CONFIGURACIÓN ===================== */
/* ↓↓↓ AQUÍ VA TU URL. Pega entre las comillas la dirección que termina
   en /exec que te dio Apps Script al implementar. Es lo ÚNICO que falta. */
...
function tituloBloque(doc, y, txt){
  if(y > PH-40){ doc.addPage(); encabezado(doc, PDF_LEV, 'Reporte de levantamiento'); y=TOPE_CONTENIDO; }
...
async function generarPDF(lev, progreso){
  ...
  /* ---------- Carátula con todos los datos ---------- */
  encabezado(doc, lev, 'Reporte de levantamiento');
  let y = TOPE_CONTENIDO;
  doc.setTextColor.apply(doc, TINTA); doc.setFont('helvetica','bold'); doc.setFontSize(17);
  doc.text('Levantamiento de sitio', 15, y); y += 7;
  doc.setFont('helvetica','normal'); doc.setFontSize(10); doc.setTextColor.apply(doc, GRIS);
  doc.text(lev.folio+'  ·  '+fmtFecha(lev.fecha), 15, y); y += 12;

  y = tituloBloque(doc, y, 'Datos generales');
  y = filaDato(doc, y, 'Proyecto',      lev.proyecto);
  y = filaDato(doc, y, 'Sitio / localidad', lev.sitio);
  y = filaDato(doc, y, 'Capturado por', lev.capturadoPor);
  y = filaDato(doc, y, 'Fecha de captura', fmtFecha(lev.fecha));
  y = filaDato(doc, y, 'Contenido',     fotos.length+' fotografías · '+videos.length+' videos');

  SECCIONES_SITIO.forEach(sec=>{
    y = tituloBloque(doc, y, sec.titulo);
    sec.items.forEach(item=>{ y = filaDato(doc, y, item.label, valorCampo(lev, item)); });
  });

  if(lev.notas && lev.notas.trim()){
    y = tituloBloque(doc, y, 'Notas y comentarios adicionales');
    y = filaDato(doc, y, 'Observaciones', lev.notas.trim());
  }
  piePagina(doc, lev, lev.folio+'  ·  Datos del sitio');
