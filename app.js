...
/* Pide al servidor el siguiente consecutivo REAL del proyecto, contando
   lo que ya existe en la hoja (registros de TODOS los usuarios).
   Con internet devuelve el número compartido; sin internet devuelve null
   y se usa un folio provisional local hasta el envío.                */
async function folioDelServidor(proyecto){
  ...
  try{
    ...
    if(d && typeof d.consec === 'number'){
      // sincroniza el contador local para futuros folios offline
      let cont={}; try{ cont=JSON.parse(localStorage.getItem('mdk_folios')||'{}'); }catch(e){}
      cont[pre] = Math.max(cont[pre]||0, d.consec);
      try{ localStorage.setItem('mdk_folios', JSON.stringify(cont)); }catch(e){}
      return { folio:'LEV-'+pre+'-'+String(d.consec).padStart(3,'0'), prefijo:pre, consec:d.consec };
    }
  }catch(e){}
  return null;
}

async function crearLev({proyecto, sitio, capturadoPor, fecha}){
  let f = await folioDelServidor(proyecto);   // intenta folio compartido
  let provisional = false;
  if(!f){ f = nuevoFolio(proyecto); provisional = true; }   // fallback offline
  return {
    id: 'lev_'+Date.now()+'_'+Math.random().toString(36).slice(2,7),
    folio: f.folio, prefijo: f.prefijo, consec: f.consec,
    folioProvisional: provisional,
...
async function borrarLev(id){
  ...
  const ok = saveLevs(getLevs().filter(l=>l.id!==id));
  recalcularContadores();
  return ok;
}

/* Reajusta cada contador al mayor consecutivo que aún exista, para que
   al borrar el último de un proyecto el folio vuelva a bajar.        */
function recalcularContadores(){
  const cont = {};
  getLevs().forEach(l=>{
    if(l.prefijo && l.consec) cont[l.prefijo] = Math.max(cont[l.prefijo]||0, l.consec);
  });
  try{ localStorage.setItem('mdk_folios', JSON.stringify(cont)); }catch(e){}
}

/* Reinicia el consecutivo de un proyecto a cero. Solo se permite si ya
   no quedan levantamientos de ese prefijo (si no, habría folios repetidos). */
function reiniciarFolio(prefijo){
  const quedan = getLevs().filter(l=>l.prefijo===prefijo).length;
  if(quedan>0) return { ok:false, quedan };
  let cont={}; try{ cont=JSON.parse(localStorage.getItem('mdk_folios')||'{}'); }catch(e){}
  delete cont[prefijo];
  try{ localStorage.setItem('mdk_folios', JSON.stringify(cont)); }catch(e){}
  return { ok:true };
...
/* ============== CORRECCIÓN AUTOMÁTICA DE REDACCIÓN ==============
   Arregla lo más común en texto escrito a prisa en campo: espacios,
   mayúscula inicial, punto final, tildes y abreviaturas frecuentes.
   No inventa contenido; solo pule lo que el técnico escribió.      */

/* correcciones de palabras frecuentes mal escritas (sin tilde, etc.) */
const CORRECCIONES = {
  'instalacion':'instalación','instalo':'instaló','instala':'instala','valido':'válido','corto':'corto','estacion':'estación','meteorologica':'meteorológica',
  'senal':'señal','camion':'camión','cimentacion':'cimentación','ubicacion':'ubicación',
  'energia':'energía','electrica':'eléctrica','electrico':'eléctrico','dia':'día','dias':'días',
  'aprox':'aprox.','poblacion':'población','comunicacion':'comunicación','construccion':'construcción',
  'aproximadamente':'aproximadamente','metros':'metros','aproximada':'aproximada',
  'acatamiento':'acatamiento','vialidad':'vialidad','señalizacion':'señalización',
  'lluvia':'lluvia','nublado':'nublado','soleado':'soleado','acceso':'acceso',
  'movilizar':'movilizar','equipo':'equipo','material':'material','base':'base',
  'porq':'porque','xq':'porque','q':'que','tmb':'también','tambien':'también',
  'asi':'así','mas':'más','solo':'solo','esta':'está','estan':'están'
};
...
function limpiarTexto(t){
  ...
  let s = String(t).trim();
  if(!s) return '';
  s = s.replace(/\s+/g,' ')           // espacios múltiples → uno
       .replace(/\s+([,.;:])/g,'$1')  // sin espacio antes de puntuación
       .replace(/([,.;:])(?=[^\s\d])/g,'$1 '); // espacio después de puntuación
  // corregir palabra por palabra respetando la puntuación pegada
  s = s.split(' ').map(w=>{
    const m = w.match(/^([¿¡("]*)(.+?)([.,;:!?)"]*)$/);
    if(!m) return w;
    const nucleo = m[2];
    const bajo = nucleo.toLowerCase();
    if(CORRECCIONES[bajo]){
      let corr = CORRECCIONES[bajo];
      // conservar mayúscula inicial si la tenía
      if(nucleo[0] === nucleo[0].toUpperCase())
        corr = corr.charAt(0).toUpperCase() + corr.slice(1);
      return m[1] + corr + m[3];
    }
    return w;
  }).join(' ');
...
/* convierte un fragmento en oración: mayúscula inicial + punto final */
function oracion(t){
  let s = limpiarTexto(t);
  if(!s) return '';
  s = s.replace(/\.+$/,'.');                 // colapsa puntos repetidos al final
  s = s.charAt(0).toUpperCase() + s.slice(1);
  if(!/[.!?]$/.test(s)) s += '.';
  return s;
}
/* pasa la inicial a minúscula (para encadenar dentro de otra frase) */
function minuscInicial(s){ return s ? s.charAt(0).toLowerCase() + s.slice(1) : s; }
...
function redactarInforme(lev){
  const d = lev.datos || {};
  /* v(): valor tal cual. vt(): valor limpiado para prosa (corrige ortografía básica). */
  const v  = k => { const x=d[k]; return (x!=null && String(x).trim()) ? String(x).trim() : null; };
  const vt = k => { const x=v(k); return x ? limpiarTexto(x) : null; };
  const si = k => v(k)==='Sí';
  const no = k => v(k)==='No';
  const P = [];
  ...
  /* --- Clima --- */
  const clima = [];
  if(si('lluvias'))  clima.push('presencia de lluvias' + (vt('lluviasDet') ? ' ('+vt('lluviasDet')+')' : ''));
  if(si('viento'))   clima.push('vientos de consideración' + (vt('vientoDet') ? ' ('+vt('vientoDet')+')' : ''));
  if(si('desbord'))  clima.push('riesgo de desbordamiento' + (vt('desbordDet') ? ' ('+vt('desbordDet')+')' : ''));
  if(clima.length){
  ...
  if(vt('serviciosDet')) ps += (ps?' ':'') + vt('serviciosDet') + '.';
  if(si('electricidad')){
    ps += (ps?' ':'') + 'Se reportan interrupciones en el suministro eléctrico' +
      (vt('electDet') ? ' ('+vt('electDet')+')' : '') +
  ...
  const inst = [];
  if(v('intensidad'))  inst.push('una intensidad de señal de ' + vt('intensidad'));
  if(v('cimentacion')){
    let c = 'cimentación tipo ' + vt('cimentacion');
    if(v('dimension')) c += ' (' + v('dimension') + ')';
    inst.push(c);
  } else if(v('dimension')){
    inst.push('dimensiones de ' + v('dimension'));
  }
  if(v('acceso')) inst.push(vt('acceso').toLowerCase());
  if(inst.length){
    let pi = 'En cuanto a la instalación del poste, se registró ' + listar(inst) + '.';
    if(vt('comentInst')) pi += ' ' + oracion(vt('comentInst'));
    if(v('tiempoSol')) pi += ' Las condiciones de asoleamiento se reportan como ' +
      vt('tiempoSol').toLowerCase() + '.';
    P.push(pi);
  } else if(vt('comentInst') || v('tiempoSol')){
    let pi = 'Respecto a la instalación del poste, ';
    if(vt('comentInst')) pi += minuscInicial(oracion(vt('comentInst'))) + ' ';
    if(v('tiempoSol')) pi += 'El tiempo de sol se reporta como ' + vt('tiempoSol').toLowerCase() + '.';
    P.push(pi.trim());
  }
  ...
  if(si('permisos')){
    pc = 'Para la ejecución de los trabajos será necesario gestionar permisos previos' +
      (vt('permisosDet') ? ' ante ' + vt('permisosDet') : '') + '.';
  } else if(no('permisos')){
  ...
  if(nf || nv){
    let pe = 'Como respaldo del levantamiento se integra' + (nf===1?'':'n') +
      ' ' + nf + ' fotografía' + (nf===1?'':'s');
  ...
  if(lev.notas && lev.notas.trim()){
    P.push('Observaciones adicionales: ' + oracion(lev.notas.trim()));
  }
  return P;
}
...
function tituloBloque(doc, y, txt){
  if(y > PH-40){
    doc.addPage(); encabezado(doc, PDF_LEV, 'Informe de levantamiento');
...
/* Párrafo de texto corrido y JUSTIFICADO, con salto de página automático */
function parrafo(doc, y, texto, tam){
  const size = tam || 10.5;
  doc.setTextColor.apply(doc, TINTA);
  doc.setFont('helvetica','normal'); doc.setFontSize(size);
  const anchoTexto = PW - 30;
  const lines = doc.splitTextToSize(String(texto), anchoTexto);
  ...
  for(let i=0;i<lines.length;i++){
    ...
    const palabras = lines[i].split(' ').filter(w=>w.length);
    // Justificar: repartir el espacio sobrante entre las palabras.
    // La última línea del párrafo y las líneas de una sola palabra van normales.
    if(!esUltima && palabras.length > 1){
      const anchoPalabras = palabras.reduce((s,w)=>s + doc.getTextWidth(w), 0);
      const espacio = (anchoTexto - anchoPalabras) / (palabras.length - 1);
      let x = 15;
      for(const w of palabras){
        doc.text(w, x, y);
        x += doc.getTextWidth(w) + espacio;
      }
    } else {
...
async function generarPDF(lev, progreso){
  ...
  /* ---------- Hoja 1: informe redactado y justificado ----------
     Mismo texto que el Word: redacción automática con corrección
     ortográfica, justificado a ambos márgenes.                  */
  encabezado(doc, lev, 'Informe de levantamiento');
  ...
  if(lev.notas && lev.notas.trim()){
    ...
    y = filaCompacta(doc, y, 'Observaciones', limpiarTexto(lev.notas.trim()), false);
  }
...
function nombrePDF(lev){ return 'Informe_'+lev.folio+'.pdf'; }
...
/* Como postJSON pero ESPERA la respuesta del servidor y reintenta.
   Se usa para los archivos: si una petición no confirma, se reintenta
   en vez de darla por buena a ciegas (así no se pierden fotos).
   Apps Script responde con redirect a googleusercontent, que sí deja
   leer el cuerpo; si el navegador lo bloquea, el reintento cubre.   */
async function postConfirm(obj, intentos){
  ...
  for(let n=1; n<=intentos; n++){
    try{
      ...
      if(txt && txt.indexOf('ok') >= 0) return true;   // el servidor confirmó
      // respondió pero sin "ok": esperar y reintentar
    }catch(e){
      // en algunos navegadores el POST cross-origin no deja leer el cuerpo;
      // si fue el último intento, lo damos por enviado para no bloquear.
      if(n === intentos) return true;
    }
    await new Promise(res=>setTimeout(res, 800*n));     // pausa creciente
  }
  return true;
}

async function enviarLev(lev, progreso){
  /* Si el folio era provisional (se creó sin internet), ahora que hay
     conexión se pide el definitivo al servidor antes de subir nada. */
  if(lev.folioProvisional){
    progreso('Confirmando folio…','Asignando el consecutivo compartido.');
    const f = await folioDelServidor(lev.proyecto);
    if(f){
      const folioViejo = lev.folio;
      lev.folio = f.folio; lev.prefijo = f.prefijo; lev.consec = f.consec;
      lev.folioProvisional = false;
      // renombrar los archivos ya guardados para que coincidan con el folio real
      (lev.medios||[]).forEach(m=>{
        if(m.name) m.name = m.name.replace(/LEV-[A-Z0-9]+-\d{3}/, lev.folio);
      });
      guardarLev(lev);
    }
  }
  ...
  const d = lev.datos||{};
  /* etiquetas legibles de cada campo, para el reporte Word */
  const camposLegibles = [];
  SECCIONES_SITIO.forEach(sec=>{
    sec.items.forEach(item=>{
      const val = valorCampo(lev, item);
      if(val && val!=='—') camposLegibles.push({ seccion:sec.titulo, label:item.label, valor:val });
    });
  });
  await postJSON({
    ...
    informe: redactarInforme(lev),        // párrafos ya redactados
    campos: camposLegibles,               // detalle etiqueta/valor
    totalFotos:fotos.length, totalVideos:videos.length,
    pdf: pdfData ? { name:nombrePDF(lev), data:pdfData } : null
  });

  /* 3. Fotos: en lotes pequeños y esperando confirmación de cada lote.
     Antes se mandaban en ráfaga con no-cors y el servidor perdía algunas
     al crear la carpeta FOTOS varias veces a la vez. Ahora van con
     confirmación real (POST legible) y el servidor las agrupa. */
  const LOTE = 3;                       // fotos por petición (equilibra peso y confiabilidad)
  const paquetesF = [];
  for(let i=0;i<fotos.length;i++){
    progreso('Preparando fotos…','Foto '+(i+1)+' de '+fotos.length+'.');
    const rec = await idbGet(fotos[i].id);
    if(!rec || !rec.blob) continue;
    paquetesF.push({ name:fotos[i].name, mimeType:'image/jpeg',
      comentario:fotos[i].comentario||'', data: await blobToBase64(rec.blob) });
  }
  for(let i=0;i<paquetesF.length;i+=LOTE){
    const lote = paquetesF.slice(i, i+LOTE);
    progreso('Subiendo fotos…', 'Fotos '+(i+1)+'–'+(i+lote.length)+' de '+paquetesF.length+'.');
    await postConfirm({ accion:'archivos', folderId:DRIVE_ID, carpeta, sub:'FOTOS',
      folio:lev.folio, archivos: lote });
  }

  /* 4. Videos: uno por petición (pesan mucho más) */
  for(let i=0;i<videos.length;i++){
    progreso('Subiendo videos…','Video '+(i+1)+' de '+videos.length+'. Puede tardar.');
    const rec = await idbGet(videos[i].id);
    if(!rec || !rec.blob) continue;
    await postConfirm({ accion:'archivos', folderId:DRIVE_ID, carpeta, sub:'VIDEOS',
      folio:lev.folio,
      archivos:[{ name:videos[i].name, mimeType:videos[i].mimeType||'video/mp4',
